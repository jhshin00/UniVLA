...
class FinetuneConfig:
    ...
    # Directory Paths
    data_root_dir: Path = Path("/ssd1/openpi_official/datasets/libero_raw")      # Path to Open-X dataset directory
    dataset_name: str = "libero_combined"                           # Name of fine-tuning dataset (e.g., `droid_wipe`)
    run_root_dir: Path = Path("runs")                               # Path to directory to store logs & checkpoints
    adapter_tmp_dir: Path = Path("adapter-tmp")                     # Temporary directory for LoRA weights before fusing
    
    # Pretraining LoRA checkpoint path
    pretrained_lora_path: Optional[str] = None                      # Path to pretrained LoRA adapter weights
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_lora:
        ...
        if cfg.pretrained_lora_path is not None:
            print(f"Loading pretrained LoRA weights from {cfg.pretrained_lora_path}")
            vla = PeftModel.from_pretrained(vla, cfg.pretrained_lora_path)
